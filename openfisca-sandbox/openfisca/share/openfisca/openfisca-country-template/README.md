...
These elements are described in different folders. All the modelling happens within the `openfisca_sandbox` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-sandbox.git
cd openfisca-sandbox
...
Substitute your package's country name for `openfisca_sandbox` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_sandbox/situation_examples/couple.json \
